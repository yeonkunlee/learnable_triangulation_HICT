...
def setup_human36m_dataloaders(config, is_train, distributed_train):
    ...
    if is_train:
        # train
        train_dataset = human36m_EXTENDED.Human36MMultiViewDataset_EXTENDED(
        ...
        train_dataloader = DataLoader(
            ...
            collate_fn=dataset_utils.make_EXTENDED_collate_fn(randomize_n_views=config.dataset.train.randomize_n_views,
    ...
    # val
    val_dataset = human36m_EXTENDED.Human36MMultiViewDataset_EXTENDED(
    ...
    val_dataloader = DataLoader(
        ...
        collate_fn=dataset_utils.make_EXTENDED_collate_fn(randomize_n_views=config.dataset.val.randomize_n_views,
...
#                 images_batch, keypoints_3d_gt, keypoints_3d_validity_gt, proj_matricies_batch = dataset_utils.prepare_batch(batch, device, config)
            ...
            keypoints_3d_extended_gt = dataset_utils.prepare_EXTENDED_batch(batch, device, config)
                ...
                ################################################
                # INTERCEPT
                keypoints_3d_gt = keypoints_3d_extended_gt
                validity_dummy = torch.ones((batch_size,8,1)).to(0)
                keypoints_3d_validity_gt = torch.cat((keypoints_3d_validity_gt, validity_dummy), dim=1)
                keypoints_3d_binary_validity_gt = (keypoints_3d_validity_gt > 0.0).type(torch.float32)
                ################################################
    ...
    if master:
        if not is_train:
            ...
            try:
                scalar_metric, full_metric = dataloader.dataset.evaluate_extended(results['keypoints_3d'])
...
#                 lr=config.opt.lr
#             )
            final_layer_list = ['final_layer.weight', 'final_layer.bias']
            final_layer_params = list(filter(lambda kv: kv[0] in final_layer_list, model.backbone.named_parameters()))
            final_layer_params = [i[1] for i in final_layer_params]
            only_backbone_params = list(filter(lambda kv: kv[0] not in final_layer_list, model.backbone.named_parameters()))
            only_backbone_params = [i[1] for i in only_backbone_params]

            opt = torch.optim.Adam(
                [{'params': only_backbone_params},
                 {'params': final_layer_params, 'lr': config.opt.volume_net_lr if hasattr(config.opt, "process_features_lr") else config.opt.lr},
                 {'params': model.volume_net.parameters(), 'lr': config.opt.volume_net_lr if hasattr(config.opt, "volume_net_lr") else config.opt.lr}
                ],
                lr=config.opt.lr
            )
