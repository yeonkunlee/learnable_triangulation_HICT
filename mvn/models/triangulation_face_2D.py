...
from mvn.models import pose_face
...
class VolumetricTriangulationNet_FACE_2D(nn.Module):
    def __init__(self, config, device='cuda:0'):
        super().__init__()

        self.num_joints = config.model.backbone.num_joints
        self.pose_face_model = pose_face.get_pose_net(device=device)
        self.backbone = self.pose_face_model.backbone
        self.keypoint_head = self.pose_face_model.keypoint_head
