...
class VolumetricTriangulationNet_EXTENDED(nn.Module):
    def __init__(self, config, device='cuda:0'):
        super().__init__()
...
class VolumetricTriangulationNet_brief(nn.Module):
    def __init__(self, config, device='cuda:0'):
        super().__init__()
    ...
    def forward(self, images, proj_matricies, batch, is_human36m = False):
        device = images.device
        batch_size, n_views = images.shape[:2]
        ...
        for batch_i in range(batch_size):
            ...
            keypoints_3d = batch['keypoints_3d'][batch_i]
            #print('yk : keypoints_3d is {}'.format(keypoints_3d.shape))
            
            #base_point = keypoints_3d[1, :3]
            base_point = keypoints_3d[6, :3]
            #base_point = np.array([0., 0., self.cuboid_side / 2])
            ...
            # transfer
            if False:  # different world coordinates
                #coord_volume = coord_volume.permute(0, 2, 1, 3)
                coord_volume = coord_volume.permute(0, 2, 1, 3) # [3, 64, 64, 64]
                inv_idx = torch.arange(coord_volume.shape[1] - 1, -1, -1).long().to(device)
                coord_volume = coord_volume.index_select(1, inv_idx)
        ...
        return vol_keypoints_3d, features, volumes, vol_confidences, cuboids, coord_volumes, keypoints_3d, heatmaps
    
#vol_keypoints_3d, features, volumes, vol_confidences, cuboids, coord_volumes, base_points
...
class AlgebraicTriangulationNet_MediaPipe(nn.Module):
    def __init__(self, config, device='cuda:0'):
        super().__init__()
        self.use_confidences = config.model.use_confidences
        config.model.backbone.alg_confidences = False
    ...
    def forward(self, images, proj_matricies, batch, numpy_keypoint_2D):
        device = images.device
        batch_size, n_views = images.shape[:2]
        n_joints = 33
        ...
        '''
        original images shape is : torch.Size([3, 3, 384, 384])
        original output heatmaps shape is : torch.Size([3, 17, 96, 96])
        original output alg_confidences shape is : torch.Size([3, 17])
        '''
        '''
        images shape is : torch.Size([3, 3, 384, 384])
        output heatmaps shape is : (3, 133, 96, 96)
        output alg_confidences shape is : torch.Size([3, 133])
        '''

        # TODO
        keypoints_2d = numpy_keypoint_2D.to(device)

        # reshape back
        images = images.view(batch_size, n_views, *images.shape[1:])
        keypoints_2d = keypoints_2d.view(batch_size, n_views, n_joints, 3)
        #print(keypoints_2d.shape)#torch.Size([1, 4, 33, 3])
        alg_confidences = keypoints_2d[:,:,:,2:3]
        alg_confidences = alg_confidences.view(batch_size, n_views, n_joints)
        #print(alg_confidences.shape)#torch.Size([1, 4, 33, 1])
        keypoints_2d = keypoints_2d[:,:,:,:2] # torch.Size([1, 4, 33, 2])
        ...
        # upscale keypoints_2d, because image shape != heatmap shape
        keypoints_2d_transformed = torch.zeros_like(keypoints_2d)
        keypoints_2d_transformed[:, :, :, 0] = keypoints_2d[:, :, :, 0]*image_shape[1]# * (image_shape[1] / heatmap_shape[1])
        keypoints_2d_transformed[:, :, :, 1] = keypoints_2d[:, :, :, 1]*image_shape[0]# * (image_shape[0] / heatmap_shape[0])
        keypoints_2d = keypoints_2d_transformed
        
        '''
        print('image_shape is {}'.format(image_shape))
        print('keypoints_2d shape is {}'.format(keypoints_2d.shape))
        print('proj_matricies shape is {}'.format(proj_matricies.shape))
        print('alg_confidences shape is {}'.format(alg_confidences.shape))
        
        image_shape is (384, 384)
        keypoints_2d shape is torch.Size([2, 3, 17, 2])
        proj_matricies shape is torch.Size([2, 3, 3, 4])
        alg_confidences shape is torch.Size([2, 3, 17])
        
        print('keypoints_2d is {}'.format(keypoints_2d))
        print('proj_matricies is {}'.format(proj_matricies))
        print('alg_confidences is {}'.format(alg_confidences))
        '''
        ...
        return keypoints_3d, keypoints_2d, alg_confidences