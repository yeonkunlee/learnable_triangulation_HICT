...
table_dtype = np.dtype([
    ...
    ('keypoints_extended', np.float32, (25,3)), # YK ADDED. EXTENDED 25 JOINTS
])
retval['table'] = []

h36m_root = sys.argv[1]

destination_file_path = os.path.join(h36m_root, "extra", f"human36m-multiview-labels-{BBOXES_SOURCE}bboxes_EXTENDEDJOINTS.npy")
    ...
    for action_idx, action in enumerate(retval['action_names']):
        ...
        for camera_idx, camera in enumerate(retval['camera_names']):
            ...
            if os.path.isdir(camera_path):
                ...
                #print('yk debug')
                #print(camera_path)
                #AssertionError: Too few frames in /datasets/human3.6m_LT/processed/S11/Directions-2/imageSequence/54138969

                if len(frame_idxs) < 15:
                    print('Too few frames in %s' % camera_path)
        ...
        # 32 joints utilized by YK.
        extended_joints = (3,2,1,6,7,8,0,12,13,15,27,26,25,17,18,19) + (14,) + (21, 22, 29, 30, 4, 5, 9, 10)
        # USED : 0 1 2 3 6 7 8 12 13 14 15 17 18 19 25 26 27
        # NOTUSED : 4 5 9 10 11 16 20 21 22 23 24 27 28 29 30 31
        with h5py.File(os.path.join(una_dinosauria_root, subject, 'MyPoses', '3D_positions',
            ...
            extended_poses_world = np.array(poses_file['3D_positions']).T.reshape(-1, 32, 3)[frame_idxs][:, extended_joints]
        ...
        table_segment['keypoints_extended'] = extended_poses_world
        ...
        #print(len(table_segment))
        #test_count += len(table_segment)
