...
class SYNTHETIC_Dataset(Dataset):
    ...
    """
    def __init__(self,
                 dataset_root='/datasets/multiview_synthetic_dataset/',
        ...
        #self.gt_dir = '/workspace/learnable-triangulation-pytorch/SYNTHETIC_results/'+self.action_name+'_alg/predicted_3d_traj.npy'
        
        self.gt_dir = self.dataset_root+self.action_name[:-18]+'.npy'
        ...
        if os.path.exists(self.gt_dir):
            ...
            print(' yk : blender GT is available. (x1000 scale)')
            GT_mat = np.load(self.gt_dir)*1000
        else:
        ...
        for camera in self.camera_idx:
            camera_name = 'camera_%02d.mat'%camera
            ...
            default_camera = Camera(R_cam, t_cam, KK, kc, 'camera_%02d'%camera)
            self.default_camera.append(default_camera)
            
            bbox_filename = 'bboxes_cam'+str(camera)+'.npy'
    ...
    def __len__(self):
        length = min(len(self.glob_list), self.GT_mat.shape[0])
        return length
